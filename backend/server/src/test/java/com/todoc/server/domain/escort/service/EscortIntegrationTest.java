...
class EscortIntegrationTest {
    ...
    @Test
    @DisplayName("동행 상세 조회 - 정상 (두 레그 모두 존재)")
    ...
    @Test
    @DisplayName("동행 상세 조회 - 존재하지 않는 신청")
    ...
    @Test
    @DisplayName("동행 진행 - MEETING → HEADING_TO_HOSPITAL (복귀시간 세팅)")
    ...
    @Test
    @DisplayName("동행 진행 - RETURNING → WRITING_REPORT (복귀시간 세팅 + Recruit.DONE)")
    ...
    @Test
    @DisplayName("동행 진행 - 허용 범위 외 상태면 예외 (PREPARING)")
